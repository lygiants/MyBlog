...
def index(request):
    start=datetime(2018, 11, 25, 0, 0, 0, 0)
    end=datetime.now()
...
def my_send_email(email, send_type="register"):
    ...
    if send_type == "register":
        ...
        # print(email_body)
        email_body = '请点击下面的链接激活你的账号: <a href="http://118.24.233.175/active/{0}" target="_blank">http://118.24.233.175/active/{1}</a>'.format(code,code)
        ...
        email_body = '请点击下面的链接重置密码: <a href="http://118.24.233.175/reset/{0}" target="_blank">http://118.24.233.175/reset/{1}</a>'.format(code,code)
